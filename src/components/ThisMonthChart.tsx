...
export default function ThisMonthChart({ data, periodLabel, productDetails }: Props) {
  if (!data.length) return (
    <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "8px", padding: "20px", textAlign: "center", color: "var(--muted)", fontSize: "13px" }}>
      데이터가 없어요
    </div>
  );
  ...
  return (
    <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "8px", padding: "18px 20px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "14px" }}>
        <div style={{ fontSize: "13px", fontWeight: 600, color: "var(--text)" }}>제품별 주문수 TOP 10</div>
        <div style={{ fontSize: "11px", color: "var(--muted)" }}>{periodLabel}</div>
      </div>
